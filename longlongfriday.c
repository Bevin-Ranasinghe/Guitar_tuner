...
volatile enum {
	...
	FILL_SECOND,
	COMPARE
} detectState = FILL_FIRST;

uint8_t lastAvg = 0, newAvg = 0;
...
volatile CircularBuffer detectStrike;
...
uint8_t averageAmplitude(CircularBuffer *Buffer) {
	if (Buffer->BufferCount == 0) return 0;
	uint8_t avg = 130;
	uint32_t sum = 0;
	uint16_t index = Buffer->tail, count = 0;
	uint8_t current = 0;

	// Now calculate average absolute deviation from the mean
	for (uint16_t i = 0; i < Buffer->BufferCount; ++i) {
		current = Buffer->buffer[(index + i) % BUFFERSIZE];
		if(current > avg){
			count++;
			sum += current;
		}
...
uint8_t peakCountOfBuffer(CircularBuffer *ADCvalueBuffer) {
	if (ADCvalueBuffer->BufferCount < 3) return 0; // Need at least 3 samples to detect a peak

	uint8_t peakCount = 0;
	uint8_t localPeaks[90] = {0};
	uint8_t localPeakCount = 0;
	uint16_t peakSum = 0;

	uint16_t index = ADCvalueBuffer->tail;
	uint16_t count = ADCvalueBuffer->BufferCount;

	for (uint16_t i = 1; i < count - 1; ++i) {
		uint8_t prev = ADCvalueBuffer->buffer[(index + i - 1) % BUFFERSIZE];
		uint8_t curr = ADCvalueBuffer->buffer[(index + i) % BUFFERSIZE];
		uint8_t next = ADCvalueBuffer->buffer[(index + i + 1) % BUFFERSIZE];

		// Check for rising edge
		if (curr > prev) {
			uint16_t flatStart = i;

			// Handle flat peak (plateau)
			while ((i + 1 < count) &&
			(ADCvalueBuffer->buffer[(index + i) % BUFFERSIZE] ==
			ADCvalueBuffer->buffer[(index + i + 1) % BUFFERSIZE])) {
				++i;
			}

			uint8_t after = ADCvalueBuffer->buffer[(index + i + 1) % BUFFERSIZE];

			if (ADCvalueBuffer->buffer[(index + i) % BUFFERSIZE] > after) {
				uint8_t peakVal = ADCvalueBuffer->buffer[(index + flatStart) % BUFFERSIZE];
				localPeaks[localPeakCount++] = peakVal;
				peakSum += peakVal;

				// Prevent buffer overflow
				if (localPeakCount >= sizeof(localPeaks)) break;
			}
		}
	}

	if (localPeakCount == 0) return 0;

	uint8_t threshold = peakSum / localPeakCount;

	for (uint8_t j = 0; j < localPeakCount; ++j) {
		if (localPeaks[j] >= threshold) {
			++peakCount;
		}
	}
...
volatile bool calcDetection = false;
volatile uint8_t take1 = 1, lastValue = 0, lastlastValue = 0;;
volatile uint16_t recording = 0;

#define RECORDING_ROUNDS 4

uint16_t getAverageZeroCrossing(void) {
	uint16_t zeroCrossings[RECORDING_ROUNDS] = {0};
	uint32_t zeroSum = 0;

	for (uint8_t round = 0; round < RECORDING_ROUNDS; ++round) {
		// Reset buffer
		cli(); // disable interrupts to avoid data race
		InitializeBuffer(&ADCvalueBuffer); // casting away volatile temporarily is safe here
		sei(); // enable interrupts

		// Fill buffer
		while (ADCvalueBuffer.BufferCount < BUFFERSIZE);

		// Compute average and zero crossing
		uint8_t avg = averageBuffer(&ADCvalueBuffer);
		uint8_t pp = MinMaxBuffer(&ADCvalueBuffer);
		uint16_t crossings = zeroCrossingBuffer(&ADCvalueBuffer, avg);

		zeroCrossings[round] = crossings;
		zeroSum += crossings;
	}

	return zeroSum / RECORDING_ROUNDS;
}
...
int main(void) {
	...
	while (1) {
		...
		if(button_pressed){
			button_pressed = 0;
			uint16_t avgCross = getAverageZeroCrossing();
			freq = avgCross/(BUFFERSIZE*0.00025);
			USART0_TransmitInt(freq);
		}
		
		/*if(button_pressed){
			button_pressed = 0;
			//takeAvgOf10RecordingsByOursTrulyCHATGBT(&ADCvalueBuffer);
			...
			//USART0_Transmit(200);
			zeroCros = countGlobalPeaks(&ADCvalueBuffer);//zeroCrossingBuffer(&ADCvalueBuffer, Our0Crossing);
